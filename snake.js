...
გამომყევი ინსტაზე: { j.gvtisavrishvili },
გამომყევი ჯითზე: { https://github.com/Gvtis09 };
...
window.addEventListener("DOMContentLoaded", function (event) {
    ...
    //   რთული
      if (event.key == "H" || event.key == "h") {
        ...
        noteElement.innerHTML = `სირთულე ჩაირთო. დააჭირე space გაგრძელებისთვის და E-ს უკან დაბრუნებისთვის!`;
      ...
      if (event.key == "E" || event.key == "e") {
        ...
        noteElement.innerHTML = `ადვილი დაბრუნდა. დააჭირე space გაგრძელებისთვის!`;
        noteElement.style.opacity = 1;
        resetGame();
        return;
      }
    ...
    function main(timestamp) {
      try {
        ...
        const pressSpaceToStart = "დააჭირე space-ს ახლიდან დაწყებისთვის.";
        const changeMode = hardMode
          ? "სირთულეს დააბრუნებ? დააჭირე E-ს."
          : "მეტი სირთულესთვის? დააჭირე H-ს.";
        const followMe =
          'და გამომყევი GitHub-ზე <a href="https://github.com/Gvtis09" , target="_blank">@Gvtis09</a>';
    ...
    function getNextPosition() {
      ...
      switch (snakeDirection) {
        case "right": {
          ...
          if (nextPosition % width == 0) throw Error("თქვენ კედელს დაეჯახეთ");
 
          if (snakePositions.slice(1).includes(nextPosition))
            throw Error("შენთავს დაეჯახე?");
          return nextPosition;
        }
        case "left": {
          const nextPosition = headPosition - 1;
          if (nextPosition % width == width - 1 || nextPosition < 0)
            throw Error("თქვენ კედელს დაეჯახეთ");
        ...
        case "up": {
          const nextPosition = headPosition - width;
          if (nextPosition < 0) throw Error("თქვენ კედელს დაეჯახეთ");
          if (snakePositions.slice(1).includes(nextPosition))
            throw Error("შენთავს დაეჯახე?");
          return nextPosition;
        }
    ...
    function getDirection(first, second) {
      ...
      throw Error("რაღაცეები არ არის დაკავშირებული");
    }
